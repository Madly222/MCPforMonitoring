...
window.fetch = async function() {
    ...
    if (typeof url === 'string' && url.startsWith(NB_API)) {
        
        await new Promise(r => setTimeout(r, 1000));
        
        const endpoint = url.replace(NB_API, '');
        let mockData = {};
        
        if (endpoint === '/scan') {
            mockData = {
                host: "10.10.20.5",
                hostname: "SW-ACCESS-BLD-3",
                device_name: "SW-ACCESS-BLD-3",
                ports: [
                    { port_name: "FastEthernet0/1", mode: "access", access_vlan: "100", description: "PC-Admin", cdp_neighbor: null, cdp_remote_port: null },
                    { port_name: "FastEthernet0/2", mode: "access", access_vlan: "200", description: "Printer", cdp_neighbor: null, cdp_remote_port: null },
                    { port_name: "FastEthernet0/3", mode: "access", access_vlan: "100", description: "User-PC", cdp_neighbor: null, cdp_remote_port: null },
                    { port_name: "GigabitEthernet0/1", mode: "trunk", access_vlan: null, description: "Uplink", cdp_neighbor: "CORE-SW-01", cdp_remote_port: "GigabitEthernet1/0/24" }
                ],
                logs: ["[10:00:01] Starting scan...", "[10:00:02] Connected via SSH", "[10:00:03] Retrieved MAC table", "[10:00:03] Found 4 ports"]
            };
        } else if (endpoint === '/scan-router') {
            mockData = {
                host: "10.10.10.1",
                hostname: "RTR-CORE-01",
                model: "Cisco 2901",
                serial: "FCZ12345678",
                ios_version: "15.4(3)M2",
                uptime: "45 weeks, 2 days",
                interfaces: [
                    { name: "GigabitEthernet0/0", ip_address: "192.168.1.1/24", status: "up", protocol: "up", description: "LAN Gateway" },
                    { name: "GigabitEthernet0/1", ip_address: "10.0.0.1/30", status: "up", protocol: "up", description: "WAN Link" },
                    { name: "GigabitEthernet0/2", ip_address: "unassigned", status: "down", protocol: "down", description: "" }
                ],
                logs: ["[10:01:01] Router scan started...", "[10:01:02] Connected via SSH", "[10:01:03] Retrieved Interfaces array"]
            };
        } else if (endpoint === '/sync-mac') {
            mockData = { 
                success: true, 
                stats: { updated: 15, skipped: 3, not_found: 2, errors: 0 }, 
                logs: ["[10:02:00] MAC table pulled", "[10:02:01] Synced 15 MACs to NetBox", "[10:02:02] Completed"] 
            };
        } else if (endpoint === '/sync-interfaces') {
            mockData = { 
                success: true, 
                stats: { assigned: 4, already_ok: 20, errors: 0 }, 
                failures: [], 
                logs: ["[10:03:00] Checking port descriptions...", "[10:03:02] Updated 4 ports on NetBox"] 
            };
        } else if (endpoint === '/apply' || endpoint === '/apply-router') {
            mockData = { 
                logs: ["[10:04:00] Preparing batch update...", "[10:04:01] Sending API requests to NetBox...", "[10:04:02] Configuration applied successfully!"] 
            };
        } else if (endpoint === '/auto') {
            const reqBody = JSON.parse(arguments[1].body);
            mockData = {
                hostname: "AUTO-SW-" + reqBody.host.split('.').pop(),
                scan_logs: ["Scan started for " + reqBody.host, "Device identified", "Ports parsed"],
                apply_logs: ["Pushing data to NetBox...", "Done"],
                errors: []
            };
        }
